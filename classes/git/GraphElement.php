...
class GraphElement {
	...
	function drawCheckoutArrowFollowBranch($actionNumber){
		...
		$branchSize=$branchOffsetNumber*self::pointMargin+(2*self::pointSize);
		if($branchOffsetNumber>0){
			//down
			$angle = (self::pointMargin+(2*self::pointSize))/$branchSize;
			...
			?><div class="arrowFollowDown <?php echo $oBranchSource->getBranchColor() ?>"
				 style="
					...
					height:<?= ($branchSize) ?>px;
					top:<?= ($oBranchSource->getBranchNumber()*self::pointMargin) ?>;
					left:<?= (($actionNumber-1)*self::pointMargin) ?>px">
